...
// Serve your static files directly from root (Option A)
app.use(express.static(path.join(__dirname)));

// Persistent session store (Render compatible)
app.use(
  session({
...
// Get bookings + blocked slots for a date
  ...
  db.get(
    ...
    (err, row1) => {
      db.get(
        ...
        (err, row2) => {
          if (row1 || row2)
            return res.status(400).json({ error: "Time already unavailable" });
